...
with st.expander("Get a Custom Prediction"):
    ...
    if predict_button:
        ...
        results = []
        countries = list(df['Country'].unique()) + ['General']
        ...
        for country in countries:
            ...
            # Store results with non-negative bounds
            results.append({
                'country': country,
                'prediction': max(0, forecast['yhat'].values[0]),
                'lower_bound': max(0, forecast['yhat_lower'].values[0]),
                'upper_bound': max(0, forecast['yhat_upper'].values[0])
            })

        # Display General Prediction with point estimate and range
        general_result = [r for r in results if r['country'] == 'General'][0]
        st.markdown(f"""
        <div class="metric-card" style="border-left: 4px solid #6c757d; margin-bottom: 20px;">
            <div class="metric-title">General {energy_type} Prediction</div>
            <div class="metric-value">{general_result['prediction']:.0f} MW</div>
            <div class="metric-title">(95% range: {general_result['lower_bound']:.0f}-{general_result['upper_bound']:.0f} MW)</div>
            <div style="font-size: 12px; color: #6c757d;">
                {pred_datetime.strftime('%Y-%m-%d %H:%M')}<br>
                {pred_temp}°C, {pred_rhum}% RH, {pred_wspd} m/s wind
            </div>
        </div>
        """, unsafe_allow_html=True)

        # Display Country Predictions as ranges only
        st.subheader("Country-Specific Ranges")
        country_results = [r for r in results if r['country'] != 'General']
        cols = st.columns(len(country_results))

        for idx, result in enumerate(country_results):
            with cols[idx]:
                st.markdown(f"""
                <div class="metric-card" style="border-left: 4px solid #2c3e50;">
                    <div class="metric-title">{result['country']} Range</div>
                    <div class="metric-value" style="font-size: 18px;">
                        {result['lower_bound']:.0f}-{result['upper_bound']:.0f} MW
                    </div>
                    <div style="font-size: 12px; color: #6c757d;">
        ...
        if energy_type == 'Solar':
            ...
            elif general_result['prediction'] < 50:
            ...
            elif general_result['prediction'] < 500:
